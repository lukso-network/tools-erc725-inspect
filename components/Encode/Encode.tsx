...
const Encode: React.FC<Props> = ({ web3 }) => {
    ...
    return (
        <>
            ...
            <Container>
                {mode === TRANSACTION_TYPES.SET_DATA ? (<EncodeSetData web3={web3} />) : null}
                {mode === TRANSACTION_TYPES.EXECUTE ? (<EncodeExecute web3={web3} />) : null}
                {mode === TRANSACTION_TYPES.TRANSFER_OWNERSHIP ? (<EncodeTransferOwnership web3={web3} />) : null}
            </Container>
        </>
    );
}
